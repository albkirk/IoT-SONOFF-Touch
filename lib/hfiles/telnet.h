...
#define MAX_TIME_INACTIVE 600000				// Maximun time for inactivity (miliseconds)
...
void telnet_loop() {
    ...
    if (telnetServer.hasClient()){
        if (telnetClient && telnetClient.connected()) {
            ...
            if (newClient.remoteIP().toString() == telnetClient.remoteIP().toString()) {
                // Reconnect
                telnetClient.stop();
            ...
            telnetClient.setNoDelay(false); // More faster
